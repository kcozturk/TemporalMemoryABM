...
##### PARAMETERS #####
seed <- 1             # Set random seed for reproducibility. Based on this each simulation will have a unique seed
...
for (z in 1:total_combinations) {
  ...
  results_list <- future_lapply(1:nr_simul, function(x) {
      ...
      if (!is.null(move)) {
        ...
        fname <- sprintf("Output/mov_decay_z%04d_sim%04d.csv", z, x)
  ...
  # Save results to CSV
  write.csv(output, file = paste0("Output/output_decay", z, ".csv"))
}
